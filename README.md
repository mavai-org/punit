...
| **punit Gradle plugin**| `org.mavai.punit` (plugin) | Auto-configures the `test` task, registers `experiment` / `exp` tasks, supports `-Prun=` filtering; resolves the shared `mavai` renderer from Maven Central and draws the reports (`punitReport`). |
...
dependencies {
    ...
    testImplementation("org.mavai:punit-report:0.11.0")
